...
band-axis subset means only those three are ever read or computed.
...
fit-small, predict-lazily:

**Fit.** k-means needs pixels in R, but not all 7.5 million of them. AEF
embeddings are built to average linearly (the read above already relies
on that with `resampling = "average"`), so fit on a coarser 120 m read
of the same tile: an overview-backed, light fetch of ~470k pixels.

``` r

emb120 <- lazy_dataset(tile, grid_from_src(tile, 120),
                       resampling = "average") |>
  lazy_map(fn = dequantize_aef, dtype = "f32")

garry_daemons()
arr <- collect(stack_bands(emb120))          # (y, x, 64)
px  <- matrix(arr, ncol = dim(arr)[[3]])
px  <- px[stats::complete.cases(px), ]
set.seed(42)
px  <- px[sample(nrow(px), 1e5), ]           # 100k px place 6 centroids fine
km  <- kmeans(px, centers = 5)
km$size
#> [1] 14675 10598 30765 21419 22543
```
...
clusters <- reduce_over(stack_bands(embeddings),
...
One fused kernel reads the full-resolution 64-band cube once, computes
every projection and the argmax per pixel, and only the finished
single-band cluster map ever reaches R. (Want the raw scores instead?
Stack plain
[`band_project()`](https://belian-earth.github.io/garry/reference/band_project.md)
