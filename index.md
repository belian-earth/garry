...
> \[!WARNING\] garry is experimental and under active development. The
> API changes without deprecation: function signatures and behaviour may
> differ between commits, and nothing here should be treated as stable
> yet. Development and benchmarking happen on Linux; macOS is CI-tested
> but less exercised, and there is a known issue with the fused
> execution path on Apple Silicon. Performance and behaviour there are
...
- **Fast.**
  [`collect()`](https://belian-earth.github.io/garry/reference/collect.md)
  plans the graph into chunks, streams reads through a GDAL daemon pool,
  and runs the fused kernels through
...
lazy: it reads and computes only at the final
[`collect()`](https://belian-earth.github.io/garry/reference/collect.md).
...
The plan itself is worth looking at.
[`plan_view()`](https://belian-earth.github.io/garry/reference/plan_view.md)
renders it as an interactive DAG (static here; live when interactive and
on the pkgdown site): stages typed by what they compute, provenance on
hover (assets, acquisition dates, reducers), and dashed edges where
bands pass through into the output. It shows the schedule you cannot see
from the code: each QA slice’s mask predicate and morphological cleanup
run as their own focal stages (halo work, kept narrow); each band’s
masking, time stack, and median fuse into a single stage; and NDVI
derives in its own stage before the write.
...
![](reference/figures/README-plan-view-1.png)
...
runs the same plan but streams each chunk into a GeoTIFF as it lands, so
nothing is ever held whole in memory. `dtype` and `scale` quantise the
f32 reflectance back to int16 (half the file size, and the scale is
written to the TIFF tags so readers recover real values); `cog = TRUE`
finalises a Cloud-Optimised GeoTIFF.
...
The same verbs work on a single raster.
...
[`write_tif()`](https://belian-earth.github.io/garry/reference/write_tif.md)
streams it to disk. See `benchmarks/compare.sh` for the back-to-back
garry-vs-ODC benchmark.
