...
.ocm_add_ch  <- function(x, v) { b <- .ocm_bc(x, v, c(length(v), 1L, 1L)); b[[1L]] + b[[2L]] }
.ocm_mul_ch  <- function(x, v) { b <- .ocm_bc(x, v, c(length(v), 1L, 1L)); b[[1L]] * b[[2L]] }
.ocm_add_col <- function(m, v) { b <- .ocm_bc(m, v, c(length(v), 1L)); b[[1L]] + b[[2L]] }
...
.ocm_edgenext_convblock <- function(x, blk) {
  ...
  h <- g_conv2d(x, blk$conv_dw$w, blk$conv_dw$b,
                padding = k %/% 2L, groups = cc)
...
.ocm_pos_embed <- function(h, w, pos) {
  hd <- 32L; temp <- 1e4; scale <- 2 * pi; eps <- 1e-6
  ...
  array(wp %*% matrix(feat, nrow = 2L * hd) + pos$b,
        c(dim(pos$w)[[1L]], h, w))
}
...
.ocm_xca_tokens <- function(xt, xca, n) {
  ...
  sm_rows <- function(a, k) {                      # stable softmax over rows
    mx <- g_max(a, dims = 2L)
...
.ocm_edgenext_sdta <- function(x, blk) {
  ...
  chunk <- function(i) g_slice_t(x, bounds[[i]] + 1L,
                                 min(bounds[[i + 1L]], cc))
  spo <- vector("list", nch)
  sp <- chunk(1L)
  for (i in seq_along(blk$convs)) {
    if (i > 1L) sp <- sp + chunk(i)
    cw <- blk$convs[[i]]
    sp <- g_conv2d(sp, cw$w, cw$b, padding = 1L,
                   groups = dim(cw$w)[[1L]])
    spo[[i]] <- sp
  }
  spo[[nch]] <- chunk(nch)
  ...
  if (!is.null(blk$pos))
    x <- x + (if (.g_traced(x))
      ...
      else .ocm_pos_embed(h, w, blk$pos))

  # XCA on tokens, gamma_xca residual
  xt <- .g_flatten_yx(x)
  xn <- .ocm_ln_tokens(xt, blk$norm_xca, cc, h * w)
  xt <- xt + .ocm_mul_col_tokens(.ocm_xca_tokens(xn, blk$xca, h * w),
                                 blk$gamma_xca)
...
.ocm_edgenext_encoder <- function(x, W) {
  ...
  for (s in 1:4) {
    ...
    if (!is.null(st$downsample)) {
      ...
      h <- g_conv2d(h, st$downsample$conv$w, st$downsample$conv$b,
                    stride = 2L)
    }
    for (blk in st$blocks) {
      h <- if (isTRUE(blk$sdta)) .ocm_edgenext_sdta(h, blk)
      else .ocm_edgenext_convblock(h, blk)
    }
    feats[[s]] <- h
  }
  feats
