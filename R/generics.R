...
S7::method(output_grid, MapNode) <- function(node, parent_grids) {
  parent_grids[[1L]]
}
S7::method(output_grid, FocalNode) <- function(node, parent_grids) {
  parent_grids[[1L]]
}
S7::method(output_grid, StackNode) <- function(node, parent_grids) {
  ...
  if (node@along %in% names(pg@dims)) {
    .garry_error(
      paste0(
        "cannot stack along existing dim `",
        node@along,
  ...
  dtype <- Reduce(
    dtype_promote,
    vapply(parent_grids, function(p) p@dtype, character(1))
  )
...
S7::method(output_grid, FusedNode) <- function(node, parent_grids) {
  parent_grids[[1L]]
}
S7::method(output_grid, WarpNode) <- function(node, parent_grids) {
  node@target_grid
}
...
.reduce_grid <- function(pg, op, over) {
  ...
  GridSpec(
    ...
    extent = pg@extent,
    dims = dims,
    dtype = .reduce_dtype(op, pg@dtype),
