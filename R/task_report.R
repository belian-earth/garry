...
#'
#' Converts the distributed scheduler's task log (option
#' `garry.task_log`; schema `time,event,key,pool,slot,mb,store_mb,ready`)
#' from a developer trace into an operator report: per-stage task counts
#' and run/queue-wait quantiles, maximum concurrency, the drain vs
#' host-tail split, and the peak measured (per-daemon anon RSS) vs
#' modelled (in-flight + resident) memory. Every diagnosis in the design
#' history parsed this CSV ad hoc; this locks the schema and answers the
#' standing "where did the time and memory go".
#'
...
garry_task_report <- function(path) {
  ...
  if (!all(need %in% names(df)))
    .garry_error(paste0(
      ...
      "no header)"), "garry_report_error")
  df$time <- as.numeric(df$time)
  events <- table(df$event)

  la <- df[df$event == "launch", c("time", "key", "pool", "slot", "mb",
                                   "store_mb", "ready")]
  ...
  q <- function(x, p) if (length(x)) round(stats::quantile(x, p, na.rm = TRUE,
                                                           names = FALSE), 3)
       else NA_real_
  stages <- do.call(rbind, lapply(split(tasks, tasks$stage), function(s)
    data.frame(stage = s$stage[[1L]], pool = s$pool[[1L]], n = nrow(s),
               run_p50 = q(s$run_s, 0.5), run_p95 = q(s$run_s, 0.95),
               wait_p50 = q(s$wait_s, 0.5), wait_p95 = q(s$wait_s, 0.95))))
  ...
  host_tail_s <- if (length(t_drain) && length(t_host))
    round(max(t_host) - max(t_drain), 3) else NA_real_

  model <- df[df$event == "model", ]
  rss <- df[df$event == "rss", ]
  peak_model_mb <- if (nrow(model))
    max(suppressWarnings(as.numeric(model$mb)) +
          suppressWarnings(as.numeric(model$store_mb)), na.rm = TRUE)
    else NA_real_
  peak_rss_mb <- if (nrow(rss)) {
    by_t <- vapply(split(suppressWarnings(as.numeric(rss$mb)), rss$time),
                   sum, numeric(1), na.rm = TRUE)
    max(by_t)
  } else NA_real_

  cli::cli_h1("garry task report: {path}")
  cli::cli_inform(c(
    "*" = paste0("events: ", paste(names(events), as.integer(events),
                                   sep = "=", collapse = ", ")),
    "*" = "{nrow(tasks)} launch/done pairs, max concurrency {max_conc}",
    "*" = paste0("drain ", drain_s, " s; host tail ", host_tail_s, " s"),
    "*" = paste0("peak modelled ", round(peak_model_mb), " MB; peak fleet ",
                 "anon RSS ", round(peak_rss_mb), " MB")))
  if (nrow(stages)) {
    cli::cli_h2("per stage (seconds)")
    for (i in seq_len(nrow(stages))) {
      s <- stages[i, ]
      cli::cli_inform(paste0(
        "  ", format(s$stage, width = 8), " ", format(s$pool, width = 5),
        " n=", format(s$n, width = 5),
        "  run p50/p95 ", s$run_p50, "/", s$run_p95,
        "  wait p50/p95 ", s$wait_p50, "/", s$wait_p95))
    }
  }
  invisible(list(events = events, tasks = tasks, stages = stages,
                 max_concurrency = max_conc, drain_s = drain_s,
                 host_tail_s = host_tail_s, peak_model_mb = peak_model_mb,
