  ...
  function(w, inputs) {
    ...
    for (id in members) {
      ...
      if (S7::S7_inherits(node, MapNode)) {
        out_pad <- min(pp)
        pv <- Map(
          function(x2, d) .trim_to_pad(x2, d - out_pad),
          pv,
          as.list(pp)
        )
        pm <- Map(
        ...
        .garry_error(
          "unexpected node on the gradient tape",
...
.grad_validate <- function(plan, wrt) {
  ...
  if (!rnode@op %in% c("sum", "mean")) {
    .garry_error(
      paste0(
        "loss reduction must be sum or mean; got ",
        rnode@op
      ),
      "garry_grad_unsupported_error"
    )
  ...
  for (id in compute@members) {
    ...
    if (S7::S7_inherits(node, ReduceNode)) {
      .garry_error(
        "chunk-local reductions on the gradient tape are not supported in v1",
        "garry_grad_unsupported_error"
      )
    }
    if (S7::S7_inherits(node, ScanNode)) {
      .garry_error(
        "scans on the gradient tape are not supported (anvl's while loop has no reverse rule)",
        "garry_grad_unsupported_error"
      )
    }
    if (S7::S7_inherits(node, PatchNode)) {
      .garry_error(
        "patch kernels (model inference) are not differentiable",
        "garry_grad_unsupported_error"
      )
    }
  }
  ...
  if (!wrt@node_id %in% compute@members) {
    .garry_error(
      "`wrt` is not part of the loss pipeline's compute stage",
      "garry_grad_unsupported_error"
    )
  }
  ...
  if (!S7::S7_inherits(wnode, FocalNode) || length(wnode@weights) == 0L) {
    .garry_error(
      "`wrt` must be a focal_kernel() LazyRaster",
      "garry_grad_unsupported_error"
    )
  }
...
lazy_value_and_grad <- function(loss, wrt, weights = NULL) {
  stopifnot(S7::S7_inherits(loss, LazyRaster), S7::S7_inherits(wrt, LazyRaster))
  plan <- plan_lazy(loss)
  ...
  src_meta <- lapply(compute@input_nodes, function(nid) {
    ...
    if (!S7::S7_inherits(node, SourceNode)) {
      .garry_error(
        "compute stage inputs must be sources on the gradient tape",
        "garry_grad_unsupported_error"
      )
    }
    src_stage <- Find(function(s) nid %in% s@members, plan@stages)
    list(node = node, chunks = src_stage@chunks, dtype = node@grid@dtype)
  })
