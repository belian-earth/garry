...
.apply_reduce <- function(op, x, margins, nan_rm) {
  switch(
    ...
    .garry_error(
      paste0("reduction op not executable: ", op),
...
.eval_node <- function(node, pv, parent_dim_names) {
  if (S7::S7_inherits(node, MapNode)) {
    ...
    if (length(node@weights) > 0L) {
      Reduce(`+`, Map(function(s2, w) s2 * w, shifts, as.list(node@weights)))
    } else {
      node@fn(shifts)
    }
    ...
    .garry_error(
      paste0(
        "node class not executable in a compute stage: ",
        class(node)[[1L]]
      ),
      "garry_plan_error"
    )
...
.trim_to_pad <- function(x, d) {
  ...
  idx <- c(
    rep(list(quote(expr = )), length(sh) - 2L),
...
.slim_fn <- function(fn) {
  ...
  env <- environment(fn)
  if (is.null(env) || identical(env, globalenv()) || isNamespace(env)) {
    return(fn)
  }
  ...
  terminal <- env
  while (
    !identical(terminal, globalenv()) &&
      !identical(terminal, emptyenv()) &&
      !isNamespace(terminal)
  ) {
    terminal <- parent.env(terminal)
  }
  if (!isNamespace(terminal)) {
    terminal <- globalenv()
  }
  ...
  for (v in unique(c(g$variables, g$functions))) {
    ...
    scope <- env
    while (
      !identical(scope, globalenv()) &&
        !identical(scope, emptyenv()) &&
        !isNamespace(scope)
    ) {
...
.compose_stage_fn <- function(
  ...
  function(inputs) {
    ...
    for (sp in specs) {
      ...
      pp <- vapply(
        node@parents,
        function(p) get(.key(p), envir = pads),
        integer(1)
      )
      ...
      if (hc > 0L) {
        ...
        out_pad <- min(pp)
        pv <- Map(function(v, d) .trim_to_pad(v, d - out_pad), pv, as.list(pp))
      }
      assign(
        .key(sp$id),
...
.reduce_partial_fn <- function(node, dim_names) {
  ...
  function(inputs) {
    ...
    part <- switch(
      ...
      mean = list(sum = g_sum(x, margins, nan_rm), count = g_count(x, margins))
    )
...
.merge_stages <- function(protos, graph, halos = NULL) {
  repeat {
    ...
    for (j in seq_along(protos)) {
      ...
      for (inp in q$inputs) {
        cons[[inp]] <- c(cons[[inp]], j)
      }
    }
    for (i in seq_along(protos)) {
      p <- protos[[i]]
      if (is.null(p) || p$kind != "compute") {
        next
      }
      cids <- unique(cons[[i]])
      cids <- cids[vapply(
      ...
      if (!.spatial_equal(p$grid, q$grid)) {
        next
      }
      ...
      input_nodes <- setdiff(unique(c(p$input_nodes, q$input_nodes)), members)
      if (.stage_halo(graph, members, input_nodes, halos) > 0L) {
        next
      }
...
.collapse_band_stacks <- function(graph, sink_ids) {
  if (!isTRUE(garry_opt("read_coalesce"))) {
  ...
  for (id in ids) {
    ...
    if (!all(vapply(ps, function(n) length(n@band) == 1L, logical(1)))) {
      next
    }
    if (
      any(vapply(
    ...
    if (
      any(vapply(
        consumers[[.key(id)]],
        function(c2) {
          S7::S7_inherits(graph_get(graph, c2), WarpNode)
        },
        logical(1)
      ))
    ) {
...
plan_lazy <- function(x) {
  ...
  if (is.list(x) && !S7::S7_inherits(x, LazyRaster)) {
    ...
    return(.plan_lazy_impl(
      graph,
      unname(sink_ids[[length(sink_ids)]]),
      sink_ids
    ))
  }
  stopifnot(S7::S7_inherits(x, LazyRaster))
  .plan_lazy_impl(x@graph, x@node_id, stats::setNames(x@node_id, "sink"))
}
...
.plan_lazy_impl <- function(graph, primary_id, sink_ids) {
  ...
  for (id in ids) {
    ...
    if (S7::S7_inherits(node, SourceNode)) {
      ...
      node_stage[[.key(id)]] <-
        new_proto("warp", id, node@target_grid, node_stage[[.key(pin)]], pin)
    } else if (
      S7::S7_inherits(node, ReduceNode) &&
      ...
      if (length(compute_sids) == 1L) {
        ...
        if (protos[[sid]]$has_focal && length(ext) > 0L) {
          ...
          node_stage[[.key(id)]] <-
            new_proto(
              ...
              id,
              .node_grid(node),
              vapply(parents, function(p) node_stage[[.key(p)]], integer(1)),
              parents,
        ...
        if (length(ext) > 0L) {
          ext_sids <- vapply(ext, function(p) node_stage[[.key(p)]], integer(1))
  ...
  for (i in seq_along(protos)) {
    if (protos[[i]]$kind == "compute") {
      protos[[i]]$halo <- .stage_halo(
        graph,
        protos[[i]]$members,
        protos[[i]]$input_nodes,
        halos
      )
    }
    protos[[i]]$out_pad <- 0L
  ...
  repeat {
    changed <- FALSE
    for (i in seq_along(protos)) {
      if (protos[[i]]$kind %in% c("source_read", "warp")) {
        next
      }
      cons <- protos[cons_idx[[i]]]
      need <- if (length(cons) == 0L) {
        0L
      } else {
        max(vapply(
          cons,
  ...
  for (i in seq_along(protos)) {
    ...
    if (s$kind == "compute") {
      ...
      s$export_pads <- .stage_export_pads(
        graph,
        s$members,
        s$input_nodes,
        s$exports,
  ...
  fuse_px_cap <- function(i) {
    if (!identical(garry_opt("placement"), "cost")) {
      return(Inf)
    }
    if (!identical(garry_opt("device"), "cpu")) {
      return(Inf)
    }
    s <- protos[[i]]
  ...
  stage_objs <- lapply(protos, function(s) {
    Stage(
      id = as.integer(s$id),
      kind = s$kind,
      members = as.integer(s$members),
      fn = s$fn,
      halo = as.integer(s$halo),
      grid = s$grid,
      chunks = .chunk_for(
        s$grid,
        .stage_block(graph, protos, s),
        s$halo,
        s$kind,
        chunk_dim,
        read_px_of[[s$id]]
...
.stage_halo <- function(graph, members, input_nodes, halos = NULL) {
  req <- if (is.null(halos)) {
    vapply(members, function(m) required_halo(graph_get(graph, m)), integer(1))
  } else {
  ...
  for (i in rev(seq_along(members))) {
    id <- members[[i]]
    consumers <- Filter(
      function(m) id %in% .node_parents(graph_get(graph, m)),
      members
    )
    downstream <- if (length(consumers) == 0L) {
      0L
    } else {
      max(vapply(consumers, function(m) H[[.key(m)]], integer(1)))
    }
...
.stage_block <- function(graph, protos, s) {
  if (s$kind == "source_read") {
    ...
    if (length(node@block_dim) == 2L) {
      return(node@block_dim)
    }
    return(c(1L, 1L))
  }
...
.stage_bytes_per_px <- function(graph, members, input_nodes) {
  ...
  in_px <- if (length(input_nodes) == 0L) {
    1
  } else {
    sum(vapply(
      input_nodes,
  ...
  sum(vapply(
    ...
    function(id) {
      ...
      if (S7::S7_inherits(n, ReduceNode)) {
        ...
        w <- tryCatch(
          get("weights", envir = environment(n@fn[[1L]]), inherits = FALSE),
...
.plan_chunk_dim <- function(graph, protos) {
  ...
  has_patch <- any(vapply(
    protos,
    function(s) {
      any(vapply(
        s$members,
        function(id) {
          S7::S7_inherits(graph_get(graph, id), PatchNode)
        },
        logical(1)
      ))
    },
