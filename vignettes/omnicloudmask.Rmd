...
SCL rides along as an ordinary band here (not the dataset's
`mask_asset`), because we want to compare masking routes rather than
commit to one; the named `resampling` keeps its class codes exact
while the reflectance bands interpolate.
...
remote <- lazy_dataset(
  ...
  resampling = c(B02 = "bilinear", B03 = "bilinear", B04 = "bilinear",
                 B08 = "bilinear", SCL = "near")
)
```
...
masking routes, per-date looks, and two composites. Rather than pull
the pixels from the Planetary Computer each time, materialise the
warped stack locally first. A `.vrt` destination writes garry's
raw-BSQ cube format: band-sequential f32 planes in a `.bin` beside a
plain VRT that any GDAL tool reads, while garry's own reader
recognises the shape and reads the bin directly (about 9x faster than
tiled GeoTIFF on repeat reads). `group_by_time()` splits the dataset
by day and `{group}` in the path names one file per date.


``` r
garry_daemons()

dir <- file.path(tempdir(), "zurich")
dir.create(dir, showWarnings = FALSE)
remote |>
  group_by_time("day") |>
  collect(path = file.path(dir, "zurich-{group}.vrt"))
basename(list.files(dir, pattern = "\\.vrt$"))
#> [1] "zurich-2024-04-05.vrt" "zurich-2024-04-08.vrt" "zurich-2024-04-30.vrt"
#> [4] "zurich-2024-05-10.vrt" "zurich-2024-05-13.vrt" "zurich-2024-05-18.vrt"
#> [7] "zurich-2024-05-20.vrt" "zurich-2024-05-25.vrt" "zurich-2024-05-28.vrt"
```

From here on, nothing touches the network. Rebuild the dataset from
the local cubes (band order and slice dates carry through), and this
time declare SCL as the `mask_asset`:


``` r
files <- sort(list.files(dir, pattern = "\\.vrt$", full.names = TRUE))
dates <- sub(".*zurich-(.*)\\.vrt$", "\\1", files)
g <- graph_new()
band_i <- function(b) stats::setNames(
  lapply(files, function(f) lazy_source(f, band = b, graph = g)), dates)
ds <- as_dataset(
  list(B02 = band_i(1), B03 = band_i(2), B04 = band_i(3),
       B08 = band_i(4), SCL = band_i(5)),
  mask_asset = "SCL")
ds
#> ── <LazyDataset> ───────────────────────────────────────────────────────────────
...
#> function (x) 
#> .ocm_infer(x, weights)
#> <bytecode: 0x5e105d9afde8>
#> <environment: 0x5e105d9af918>
#> 
#> $kernel_id
...
Same date, both masks applied, masked pixels in orange (`preview()`'s
...
but bright cloud fragments are embedded in it, and patches of
perfectly clear city have been thrown away; OmniCloudMask keeps the
genuinely clear corridors and removes cloud and shadow together. Over
a stack this costs SCL twice: fewer clean observations per pixel AND
...
and on a time series the per-scene inferences pipeline across the
daemon pool. Second, for composites the natural next step is the
multivariate route: `reduce_over(cube, geomedian(), over = "t")` on the
OCM-masked stack gives spectra no per-band median can (see the
composite vignette).
