...
with garry: download them once with `ocm_fetch_weights()` (about 58 MB,
hash-verified) and `ocm_model()` finds them automatically. It can also
read the Python package's download cache or any directory you point it
at (see `?ocm_load_weights`).

This vignette reproduces the classic OmniCloudMask demonstration
(after the OCM article in the vrtility package) on garry: Zurich in
spring, with scenes up to
...
the pixels from the Planetary Computer each time, checkpoint the
warped stack locally: `materialise()` executes the graph into garry's
raw-BSQ cube format (one multiband `.vrt` + `.bin` per date, readable
by any GDAL tool and re-read by garry much faster than tiled
GeoTIFF) through ONE plan, and returns the same lazy dataset rebuilt
over the local files with band names, slice dates, and the
`mask_asset` intact. By default the cubes land in a unique
