...
Runs the placement pass (see `design/placement-cost-pass.md`) over the
plan the same way `collect(distributed = TRUE)` would, and returns its
decision table: one row per fusable source -\> compute chain with the
decision, the modelled costs (cost mode), and the reason. Placement
depends on runtime resources, so pool widths are read from the live
